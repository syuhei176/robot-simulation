...
- **ブラウザ ビューア** — `stair.html` で **機構（蛇/四足）× モーター（5機種）× 総質量** を切り替えて挙動を再生。
- **平面歩容の学習（RL）** — 平面の蛇型で前進歩容を自前 PPO で学習するデモ（`index.html`）。
...
- `http://localhost:5173/` … 平面蛇型の歩容学習（手動 CPG / PPO）デモ
- `http://localhost:5173/stair.html` … 機構 × モーター × 総質量 のビューア
  - 蛇＝階段 physical attempt（赤: 落下/干渉、オレンジ: トルク超過、黄: 滑り、水色: 支持接触）
  - 四足＝3D 動的歩行（モーター選択で τ上限を自動設定、総質量スライダーで「動く重さ」を探索）
