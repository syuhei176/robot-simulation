...
- **3D 四足の強化学習（end-to-end）** — `runQuadrupedGait` を 1 制御ステップずつ進められる
  RL 環境 `QuadEnv`（`src/env/QuadEnv.ts`）に展開。方策が 8 関節（hip×4 + knee×4）の目標角を直接出力し、
  忠実トルク（±cap clamp）＋横安定化の物理で平地の小型四足を歩かせるのを学ぶ。観測＝位相クロック＋胴 pitch/速度
  ＋関節角、報酬＝前進−傾き−行動エネルギー−トルク飽和。学習は Mac オフライン（`Policy`+`PPO`/TF.js）で、
  方策の重みを `public/policies/*.json` に保存する。`pnpm train-quad`
...
pnpm train-quad     # 3D 四足を end-to-end RL(PPO)で学習 → public/policies/*.json（--iters/--rollout/--motor/--mass）
```
